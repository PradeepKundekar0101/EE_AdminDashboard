import { Button, Carousel, CarouselProps, Drawer, List, Tag, Rate ,Form as AntdForm, Input, message, Upload} from "antd";
  ...
  return (
    <CustomLayout>
      ...
      >
        <div className=" border-b-[0.5px] border-slate-300 mb-3">
          <div className="flex justify-between">
            ...
            {selectedJournal?.reviewId ? (
              <Tag color="green" className="flex items-center">{"Reviewed By "+selectedJournal?.review.userId}</Tag>
            ) : (
              <div>
        ...
        <div>
          ...
          <div>
            ...
            {!selectedJournal?.reviewId ? (
             ...
             <div>
              <h1 className="w-full px-3 py-2 rounded-md bg-slate-100 border-[0.5px] border-slate-300">{selectedJournal?.review?.value}</h1>
              {selectedJournal?.review && <div className=" flex items-center my-3 space-x-2"><Rate disabled value={selectedJournal?.review?.rating}/> <Tag>{selectedJournal?.review?.rating+" stars"}</Tag> </div>}
              </div>
            )}
          </div>
        </div>
        {selectedJournal && selectedJournal?.review &&<div>
          <Upload>
        ...
        >
          <h1 className="text-xl border-b-[0.4px] border-slate-300 pb-2 mb-2">Add Review</h1>
